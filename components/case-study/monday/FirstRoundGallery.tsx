...
// usual portrait SummaryCard tiles. Slide 1 (207:5668) is empty in the
// design itself; slides 2-4 have real exported images — each supplied
// as a genuine light/dark pair (the wordmark/icon marks are white-on-
// dark and black-on-light versions of the same mark, not a color a CSS
// filter could derive), matching primitives.tsx's own Exhibit darkSrc
// convention.
const ALIGN_INSET = "max(1rem, calc((100vw - 512px) / 2))";

const SLIDES: { id: string; alt: string; src?: string; darkSrc?: string; bordered?: boolean }[] = [
  { id: "1", alt: "First round exploration 1" },
  {
    id: "2",
  ...
  return (
    <div className="flex w-full flex-col gap-3">
      ...
      <CardRow
        ...
        items={SLIDES.map(({ id, alt, src, darkSrc, bordered }) => ({
          id,
          content: <GalleryCard alt={alt} src={src} darkSrc={darkSrc} bordered={bordered} />,
        }))}
      />
    </div>
  );
